...
function Write-MSPLog {
    ...
    # Structured JSON-line sidecar -- SIEM/Log analytics friendly.
    # Lives next to the text log with a .jsonl extension. We only emit it
    # when file logging is on; one record per line.
    if (-not $NoFile -and $Script:Config.logging.logToFile -and $Script:LogPath) {
        try {
            $jsonPath = [System.IO.Path]::ChangeExtension($Script:LogPath, '.jsonl')
            ...
            Add-Content -Path $jsonPath -Value $record -ErrorAction SilentlyContinue
        } catch { }
    }
}
...
function Show-MSPBanner {
    ...
    ║                                                                      ║
    ║          ╔══════════════════════════════════════════════════╗        ║
    ║          ║  Cross-Platform MSP Diagnostics & Automation v$Version     ║
    ║          ║         Discover. Diagnose. Deploy.              ║        ║
    ║          ╚══════════════════════════════════════════════════╝        ║
    ║                                                                      ║
    ╚══════════════════════════════════════════════════════════════════════╝
